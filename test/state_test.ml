...
let%test_module "STATE.make" =
  (module struct
    ...
    let level = LEVEL.make bricks box "level"
...
let%test_module "STATE.update" =
  (module struct
    ...
    let%test "update state playing to game over" =
      let state_playing = { state with status = Playing; ball = BALL.{ state.ball with pv = 0 } } in
    ...
    let%test "update state playing to victory" =
      let bricks_dead = [] in
      let level_dead = LEVEL.make bricks_dead box "level_dead" in
...
let%test_module "STATE.is_alive" =
  (module struct
    ...
    let%test "state is not alive" =
      let state_dead = { state with ball = BALL.{ state.ball with pv = 0 } } in
